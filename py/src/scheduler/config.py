...
class Config(object):
    def __init__(self) -> None:
        ...
        self.pwd = os.path.join(basePath,'ComfyUI_Scheduler','config')
        self.reload()

    def _load_config(self) -> dict:
        try:
            with open(f"{self.pwd}/config.yaml", "rb") as fp:
                yconfig = yaml.safe_load(fp)
        except FileNotFoundError:
            shutil.copyfile(f"{self.pwd}/config.yaml.template", f"{self.pwd}/config.yaml")
            with open(f"{self.pwd}/config.yaml", "rb") as fp:
                yconfig = yaml.safe_load(fp)
    ...
    def save_config(self):
        ...
        yconfig["config"] = self.config
        with open(f"{self.pwd}/config.yaml", 'w', encoding='utf-8') as file:
